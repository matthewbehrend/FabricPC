...
### Implementation findings (2026-09-08)

Two findings from implementing deliverables 1 to 4 on this branch. Both are recorded in `docs/reports/epc_regime_and_stability_report.md` (Sections 5.7 and 5.8); the first is also recorded under Design 1 of `docs/dev_plans/epc_review_fixes_lanczos_regime_probe.md`.

**Finding 1: the breakdown guard alone does not keep unexcited modes out of λ_min; an eps weight floor does.** The guard fires on the three-mode fixtures of the Evidence table but not on the `stability` section's depth-5 chain (x32 → 5 × h32 → y10, batch 8, 160 ε coordinates, 10 excited modes). In float32 the rounding accumulated over ten steps kept β_10 above √eps(dtype)·max(max_i |α_i|, max_i β_i), the recurrence ran all 30 steps on that noise, and the unexcited floor λ = 1.000 appeared as a Ritz value and was reported as λ_min against the excited minimum 1.726; λ_max and f̄ were unaffected (relative errors 1.9e-7 and 2.8e-9). The floor ghosts carried Ritz weights of 3.6e-18 and 4.8e-14 against 6.3e-4 on the smallest excited mode. Fix: `lanczos_extremes` takes λ_max and λ_min over the Ritz modes whose weight w_k exceeds eps(dtype) (1.2e-7 in float32), the cutoff the guard expresses through β, since a direction entering with β/|α| below √eps carries weight below eps in g0; `ritz_values` and `ritz_weights` are returned unfiltered. After the fix λ_min on that chain is 1.72621 against the oracle's 1.72621 (relative error 4.3e-8). Pinned by `tests/test_epsilon_spectrum.py::TestLanczosExplicitMatrix::test_weight_floor_hides_the_unexcited_floor` (H = I + JᵀJ with J of rank 10 on 40 coordinates, started at g0 = Jᵀr) and by the float32 and float64 comparisons against the oracle's excited extremes and weighted fraction on four fixtures in `tests/test_linear_pc_oracle.py`.

**Finding 2: on the resnet18 the excited band is not compact, and the 2-epoch accuracy follows f_max, not f̄.** `scripts/epc_analysis.py --resnet18` (64-sample test batch, 30 Lanczos steps, 17 s including compile) at init: λ_max = 16.45 (Ritz residual 4.6e-4, the value power iteration gave); λ_min = −0.42 (residual 0.11, so a sign and an order of magnitude, not a converged value), so H_ε is indefinite already at init, with 1.2% of ‖g0‖² on negative curvature, which grows by 1.002 over the defaults' five steps; f̄ = 0.010 at the defaults against f_max = 0.080, so the weight-averaged eigenvalue is about 2, near the precision floor, while λ_max is 16.45. The sentence under "Why the weights, not only the extremes" above, that f̄ agrees with the slowest-excited-mode condition "whenever that band is compact, which is the resnet18's case", is not supported by the measurement, and the sweep letters computed from f̄ no longer track the recorded accuracy: at η = 0.01, T = 3 to 5 (36.9% to 35.0%, on the way to the 31% plateau) f̄ is 0.06 to 0.09 and reads backprop-like; at η = 0.03, T = 5 to 64 (31.4% to 31.2%, on the plateau) f̄ is 0.23 to 0.88 and reads partially relaxed. f_max = f(16.45) tracks the accuracy in every row, and the single-eigenvalue fit lands near λ_max (λ_eff = 12.0) for the same reason. Reading: the bulk of the gradient weight sits on modes with λ near 1 or 2, whose relaxation moves each hidden error from −η·T·g toward −g/λ, a change of magnitude by about 1/(η·T) with the direction preserved, which Adam's per-parameter normalization absorbs; the top modes couple the hidden errors to the output prediction, and their relaxation re-weights the output error by S⁻¹, a matrix rescaling Adam cannot undo. Disposition: the `Regime` band stays on f̄ as designed, because f̄ is the equilibrium criterion (the energy is at its minimum only when the modes carrying the gradient have relaxed); f_max is reported beside it (`Regime.f_max`, the second number in `str(regime)`, the third field per sweep cell in `--resnet18`, a column in the report's Section 5.8 table). Open question, not decided: a two-sided band, "backprop-like" on f_max < 0.1 (the quantity the accuracy followed on this graph) and "near PC equilibrium" on f̄ > 0.9 (the energy criterion), which would restore the rejected "bands on f_max" for the backprop side only. The measurement is one batch at one seed.

**Finding 3: the control runs select the second case of the reading rule; the weight norms fall in every run.** The four runs of deliverable 5 (2026-09-08, RTX 3090, seed 42, the normalized trainer, probed every 50 updates on the 64-sample test batch; report Section 5.9): (1e-3, 5) reproduced the collapse with the reversal flag at update 2700 (epoch 14), η·λ_max > 2 at 2750 (epoch 15), and chance at epoch 15, λ_max going 22 → 27 (epoch 8) → 103 (12) → 1 632 (14) → 31 060 (15) → 1.0 (dead); (1e-3, 1) reached 67.7% with λ_max 22 → 29 and no flag; backprop reached 69.0% with λ_max 22 → 24; (1e-2, 1) flagged the reversal at update 1150 (epoch 6, one probe after the predicted 1100, where η(λ_max − 1) was 0.85), crossed at 1200 with λ_max = 6 132 and λ_min = −354 where the old estimator returned −9 399, and was at chance in epoch 7. The total Frobenius weight norm fell from 1670 to between 1597 and 1641 in every run, every convolution kernel's norm by 1.5 to 1.7% over the collapse epochs, so λ_max (a spectral quantity, 1 + σ_max(J)²) grew 74× while the weight scale shrank under weight decay 1e-2. Reading: λ_max drifts about 1.01× per epoch regardless of the solver and runs away only in the ePC cells with the larger η·T; ePC's relaxation feeds the growth, and the remedy is on the solver side (a rate that follows λ_max, or η·T kept in the backprop regime), not weight-norm control. On the band question of Finding 2: during training f̄ crossed 0.1 two to three epochs before each collapse's reversal (epoch 11 of 14 at the defaults, epoch 4 of 6 at (1e-2, 1)) while f_max had crossed it in epoch 5 and epoch 1 respectively with accuracy still improving, so f̄ is the tracked early warning and f_max the predictor of the 2-epoch accuracy penalty at init. The trackers were also reconciled with the `IterContext` PR: `make_inference_history` iterates `segments()` inside one jitted program and passes each sample through `finalize_state`, and `make_tracked_probe(structure)` is the per-step-metrics twin of `make_tracked_settle`, used from `train`'s iteration callback by the compare script's checkpoint probes.
...
| `epsilon_spectrum`, `Regime`, `RegimeProbe`, `--track_regime`, `--plot_track`, control runs | deliverables 1 to 5 done on the rebased branch (suite 654 passed, 6 skipped, none of them this work's); the probe's `train()` integration tests run, the demo's `--track_regime` path is verified, and the four control runs are recorded in Finding 3 above and in the report's Section 5.9                                                    |
