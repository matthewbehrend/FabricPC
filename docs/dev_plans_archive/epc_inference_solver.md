...
| `epsilon_spectrum`, `Regime`, `RegimeProbe`, `--track_regime`, `--plot_track`, control runs | deliverables 1 to 4 implemented (suite 631 passed, 7 skipped); the probe's `train()` integration test and the demo's `--track_regime` path wait on the `IterContext` PR; deliverable 5 (the control runs) pending. Two implementation findings recorded above                                                                     |
