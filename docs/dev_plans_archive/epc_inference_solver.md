...
New node-level override point — a sibling method beside `forward_and_latent_grads` in `fabricpc/nodes/base.py`, not a mode on it — dispatched as `node_info.node_class.forward_from_error(...)`:
