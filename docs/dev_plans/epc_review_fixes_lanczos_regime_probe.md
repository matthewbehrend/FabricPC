...
All of these were measured on 2026-09-04 with batch-summed weight gradients. Release 0.5.1 (2026-09-07, commit ba7e3d6) then made the trainer divide the weight gradients by the prediction count (`pc_weight_gradients`, `grad_denominator`), and the branch was rebased onto it. The inference energy is untouched: `error_energy` still sums per-sample energies, so H_ε, λ_max, and the regime at init are the same under both trainers. The training trajectories are not: the sweep, the 100-epoch runs, and the two tracking CSVs are accepted as recorded and are not re-collected. Design 4's control runs are the first tracking data from the normalized trainer.
...
| negative_weight | Σ_{θ_k < 0} w_k, the fraction of ‖g0‖² on negative-curvature modes |
...
- The oracle docstring does not say that cyclic graphs are outside it, nor that a batch's λ_max is the per-sample maximum (`error_energy` sums per-sample energies, `inference_epc.py:213-216`, so H_ε is block-diagonal over samples).
...
`__str__` is the one-line label with precedence: `unstable`, then `growth_min > 1.1` ("indefinite: negative curvature carrying X% of the gradient grows Y× over T steps"), then the band with f̄, f_max, and the reversal note. The band uses positive modes only; negative-curvature modes have no equilibrium to relax toward and are judged by `growth_min`, not by the band.
...
Scope of the reversal flag. The formula is the linear unit-precision chain's (F3). On a nonlinear graph it is the local quadratic model's prediction at ε = 0, and the (1e-2, 1) control run (Design 4) tests it: the flag should fire at update 1100, one epoch before that run reached chance.
...
CSV: metadata columns first, constant per row (`trainer, eta_infer, infer_steps, every, probe_batch`; η and T empty under backprop), then `update, epoch, lambda_max, lambda_min, f_weighted, f_max, negative_weight, growth_min, residual_max, residual_min, eta_lambda_max, output_gradient_reverses, unstable, train_energy, test_accuracy`, then `wnorm:<edge_key>` per weight. Readers take η, T, and the trainer from the columns; the filename is a label. Works under `algorithm="backprop"` (spectrum and norms; regime columns empty). The docstring states the cost: supplying `iter_callback` forces a device sync on every batch (`trainer.py:570`), not only on probed ones.

The probe consumes the `IterContext` PR: `iter_callback(ctx)` with `params`, `opt_state`, `step`, `batch`, and `metrics`. The training step donates the parameter buffers, so `ctx.params` is valid only during the callback; the probe reads it there and stores floats, copying nothing. That PR migrates every three-argument caller in the same change: `fabricpc/utils/dashboarding/callbacks.py`, `examples/transformer_v2_demo.py`, `fabricpc/tuning/bayesian_tuner.py:111`, and the trainer and `test_fabricpc.py` tests.
...
- `linear_pc_oracle.py`: the docstring gains the cyclic-graph limit and the per-sample-maximum sentence; `stability_bound(H)` raises `ValueError("no positive curvature ...")` when λ_max ≤ 0. Part 2b (`make_top_epsilon_eigenvalue`, `top_epsilon_eigenvalue`) is deleted in the same commit as `regime_label`, with every caller migrated in that commit: the demo's `lambda_max_at_init` and settings line (D), the compare script (E), `scripts/epc_analysis.py` at `regime_letter`, `--resnet18`, `stability`, the gelu bracket, and `section_track_lambda_max` (F), and the oracle, parity, and regime tests (G). The module becomes NumPy-only.

### B. `fabricpc/core/inference_epc.py`

`Regime` and `regime(spectrum)` as in Design 2; `regime_label` removed with its callers (listed under A). Class docstring: the equilibrium condition "η·T·λ ≳ 3 on the modes that carry the gradient, f̄ > 0.9; on a linear graph these are the eig(S) modes"; the T = 1 sentence replaced by the reversal mechanism (F3); the Adam sentence with the ε caveat and the SGD disparity (F6); λ_max = 1 + σ_max(J)² as a weight-scale quantity controlled by normalization or weight decay; Goemaere et al.'s ResNet-18 setting and the differences (F5).
...
- `stability`: on the depth-5 chain, λ_max against `eigvalsh` max, λ_min against the minimum of `excited_eigenvalues`, f̄ against the oracle's weighted fraction Σ_{λ > 0} (v_λᵀg0)² f(λ) / Σ_{λ > 0} (v_λᵀg0)² over the excited modes, v_λ the unit eigenvector of eigenvalue λ; gelu bracket unchanged.
...
- `docs/dev_plans/epc_reviewer_response_oracle_and_analysis.md`: a dated revision note under Context pointing to the oracle review and this plan; B.2 and D.5 amended.
- `docs/reports/epc_regime_and_stability_report.md`: line 5 (the revision note: the Section 5.8 tracking and the growth phases now come from the normalized trainer's control runs, the sweep and the 100-epoch runs were not repeated, so "every measurement in this report is unchanged" is replaced by that distinction), lines 12–13 (T = 1 mechanism; growth phases from the re-run), 61 (the 1 < ηλ < 2 row gains the odd-T output reversal), 77–82 (the regime table: bands on f̄, the equilibrium condition, a `Regime` column in place of `regime_label`), 142 (the muPC bracket now pins the claim), 165 (Adam-ε caveat), 249 ("slowest 0.5%" → f̄ and λ_min at init), 307 (the negative value was the estimator's defect; the re-run's value), 309 and 321 (control-run outcomes; growth phases), 327, 334, 340; regenerate the Section 5.8 sweep letters from `--resnet18`; add a subsection on the paper's ResNet-18 setting and one on the S⁻¹r damping.
...
The guarded rows used a fixed tolerance. The design's threshold √eps(dtype)·max(max_i |α_i|, max_i β_i) is about 1.2e-2 in float32 and 5e-7 in float64 on this spectrum (max Ritz value 33.6), above β_3 in both precisions, so it also stops at step 3.
...
Relaxed fractions at η = 0.03, T = 10. The f̄ columns are the first-draft review's sum Σ_{θ_k > 0} w_k f(θ_k) without the division by Σ_{θ_k > 0} w_k. At weight std 1.0 no mode is negative and the two definitions coincide. At std 2.0 the design's normalized f̄ is 0.458 / 0.774 ≈ 0.59; it has not been re-measured, and the exact-versus-Lanczos agreement below is for the unnormalized sum:
...
1. A without the Part 2b deletion: the new module, the oracle docstring and `stability_bound` edits, and G's spectrum test. The old estimator and label still work, so no caller moves.
2. B, the Part 2b deletion, and every caller of both in one commit: D's `spectrum_at_init` and settings line; E; F's `regime_letter`, `--resnet18`, `stability`, and gelu-bracket migrations and the deletion of `section_track_lambda_max` with its arguments; G's oracle test rename, parity test, and `TestRegime`. `--plot_track` keeps reading the old CSVs until step 3.
3. After the rebase: C and the probe test; D's `--track_regime`, `--schedule_epochs`, and docstring; the rest of F (the `--plot_track` reader, the S⁻¹r table, the normalized gradients).
4. H.
