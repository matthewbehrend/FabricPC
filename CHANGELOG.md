...
- `scripts/epc_analysis.py` — the backprop regime, equilibrium energy profiles, convergence spectra, and stability on linear graphs (CPU, under a minute), with GPU options that measure λ_max on the resnet18 demo (`--resnet18`) and track eta_infer·λ_max during training (`--track_lambda_max N`).
