...
- `scripts/epc_analysis.py` — the backprop regime (gradients through the trainer's per-prediction normalization), equilibrium energy profiles with the Theorem 1 damping ‖r S⁻¹‖/‖r‖ per depth and init, convergence spectra, and stability (Lanczos against the oracle's excited extremes and f̄) on linear graphs (CPU, under a minute); `--resnet18` measures the spectrum at init on the demo and prints f̄ per recorded sweep cell; `--plot_track` renders `RegimeProbe` CSVs. `examples/resnet18_cifar10_demo.py --track_regime N` records the spectrum during training (the four control runs of `docs/dev_plans/epc_review_fixes_lanczos_regime_probe.md`).
